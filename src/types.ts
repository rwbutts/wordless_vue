...
export const enum EventNames {
    KB_RAWKEY = "kb_raw_key",
    TRIGGER_WORD_LOAD = "load_word",
...
export interface ILetterColorPair {
    letter: string;
    color: MatchCodes;
}
...
export class LetterColorPair {
    ...
    static createScoredLetter(
        guessLetter: string,
        answer: string,
        guessColumn: number
    ): LetterColorPair {
        const color =
            guessLetter === answer.charAt(guessColumn)
                ? MatchCodes.CORRECT
    ...
    static create(
        letter = "",
        color: MatchCodes = MatchCodes.DEFAULT
    ): LetterColorPair {
        return new LetterColorPair(letter, color);
    }
}

export interface IClassifiedKeyRefs {
    all: string[];
    alpha: string[];
    nonalpha: string[];
}
